...
if(_maxBuyIndex != -1 && _buyIndex != -1) then {
	...
	if(_maxBuyPrice != 0 && _buyPrice != 0) then {
		_takePrice = (_maxBuyPrice/100) * life_item_modifier; //1 Percent
		_newBuyPrice = _buyPrice - (_takePrice * _count);
	};
};
...
if(_maxSellIndex != -1 && _sellIndex != -1) then {
	...
	if(_maxSellPrice != 0 && _sellPrice != 0) then {
		_takePrice = (_maxSellPrice/100) * life_item_modifier; //1 Percent
		_newSellPrice = _sellPrice - (_takePrice * _count);
	};	
};
...
if(_newBuyPrice != -1 && _newSellPrice != -1) then {
	if(_newBuyPrice < _newSellPrice) exitWith {
		dynamic_sell_array set[_maxSellIndex,[_item,1]];
		dynamic_buy_array set[_maxBuyIndex,[_item,1]];
		_exit = true;
	};
};
if(_exit) exitWith {};
...
if(_newSellPrice >= 1 && _maxSellPrice != -1) then {
	if(_newSellPrice <= _maxSellPrice) then {
		dynamic_sell_array set[_maxSellIndex,[_item,_newSellPrice]];
	} else {
		dynamic_sell_array set[_maxSellIndex,[_item,_maxSellPrice]];
	};
};

if(_newBuyPrice >= 1 && _maxBuyPrice != -1) then {
	if(_newBuyPrice <= _maxBuyPrice) then {
		dynamic_buy_array set[_maxBuyIndex,[_item,_newBuyPrice]];
	} else {
		//Set max as it needs to be.
		dynamic_buy_array set[_maxBuyIndex,[_item,_maxBuyPrice]];
	};
};
