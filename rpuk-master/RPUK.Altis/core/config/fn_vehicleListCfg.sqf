...
//************ POLICE VEHICLE SHOPS
	...
	case "cop_car":
	{
		...
		if(__GETC__(life_plodrank) >= 8) then //DI AND ABOVE
		{
...
//			_return pushBack ["B_MRAP_01_F",0];
	...
	case "cop_airhq":
	{
		...
		{
			_return pushBack ["B_UAV_01_F",0];
			_return pushBack ["O_Heli_Transport_04_bench_black_F",0];
			_return pushBack ["O_Heli_Transport_04_covered_black_F",0];
			_return pushBack ["I_Heli_light_03_unarmed_F",0];
		};
		if(call life_fnc_isAR || call life_fnc_isNPAS) then // || call life_fnc_isNIU
		{
			_return pushBack ["B_UAV_01_F",0];
		};
	};
