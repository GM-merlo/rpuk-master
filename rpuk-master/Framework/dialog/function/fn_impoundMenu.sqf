...
_control ctrlSetText format["%1 - %2/20 Vehicles",localize "STR_GUI_Garage",count _vehicles];
...

{
	...
	if(_vehicleInfo select 2 != "pictureThing") then {_control lbSetPicture [(lbSize _control)-1,_vehicleInfo select 2];}; //If not storage crate show image.
