...
if(_uniform == "U_NikosAgedBody" && !(life_mayor_settings select 0 == getPlayerUID player)) then {_uniform = "";};
if(_uniform in [
    ...
    "U_O_FullGhillie_ard"
] && side player != independent) then {_uniform = "U_I_CombatUniform";};
...
life_maxWeight = 100; // this is madness... why is max virtual carryweight being set here only to be overridden later? NDJ 21/07/16
{
    _item = [_x,1] call life_fnc_varHandle;
    [true,_item,1] call life_fnc_handleInv;
} foreach (_yItems);
life_maxWeight = 24; // this is madness... why is max virtual carryweight being set here? NDJ 21/07/16
