...
with missionnamespace do {
	...
	if((isServer && isDedicated) || !hasInterface) then {
		ciaran_fnc_MP_packet = [0,_params,_functionName,_target,_isPersistent,_isCall,"__SERVER__","__SERVER__"];
	} else {
		ciaran_fnc_MP_packet = [0,_params,_functionName,_target,_isPersistent,_isCall,profileName,getPlayerUID player];
	};
	publicvariableserver "ciaran_fnc_MP_packet";
