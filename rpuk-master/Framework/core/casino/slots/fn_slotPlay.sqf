...
if(RPUK_dosh >= _cost) then
{
	...
	{
		RPUK_dosh = RPUK_dosh + _winAmount;
...
{
	if(RPUK_dosh < _cost) exitWith {hint (format["You need £%1 to play this machine.",_cost]);
	life_is_processing = false;};
};
