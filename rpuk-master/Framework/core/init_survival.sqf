[] spawn {
	...
	_fnc_food =
	{
		if(life_hunger < 2) then {[player,"Fainted due to starvation."] call life_fnc_medilogSubmit; player setHitPointDamage ["HitBody",1]; hint localize "STR_NOTF_EatMSG_Death";}
		else
		{
	...
	_fnc_water =
	{
		if(life_thirst < 2) then {[player,"Fainted due to dehydration."] call life_fnc_medilogSubmit; player setHitPointDamage ["HitBody",1]; hint localize "STR_NOTF_DrinkMSG_Death";}
		else
		{
	...
	while {true} do {
		...
		if(!(currentWeapon player == "" || currentWeapon player == "binocular")) then {
			if(!(life_fed_hacked)) then {
				...
				[[1,0,2],format["A camera at the federal reserve has detected a intruder with a weapon at the Bank's Vault!"]] remoteExec ["life_fnc_broadcast",west];
			} else {
