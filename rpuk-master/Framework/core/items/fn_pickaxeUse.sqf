...
switch (true) do
{
	...
	case (player distance (getMarkerPos "salt_1") < 120) : {_mine = "salt"; _val = 1;};
	case (player distance (getMarkerPos "sand_1") < 75) : {_mine = "sand"; _val = 1;};
	...
	case (player distance (getMarkerPos "rock_1") < 50): {_mine = "rock"; _val = 1;};
	default {_mine = "";};
};
//Mine check
if(_mine == "") exitWith {hint localize "STR_ISTR_Pick_NotNear"};
if(vehicle player != player) exitWith {hint localize "STR_ISTR_Pick_MineVeh"; life_action_inUse = false;};

_diff = [_mine,_val,life_carryWeight,life_maxWeight] call life_fnc_calWeightDiff;
if(_diff == 0) exitWith {hint localize "STR_NOTF_InvFull"; life_action_inUse = false;};
