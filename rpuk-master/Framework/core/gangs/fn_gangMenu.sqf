...
if(life_my_gang_rank < 3) then {
	...
	(getControl(2620,2632)) ctrlEnable false; //Player selection box.
	(getControl(2620,2633)) ctrlEnable false; //Update Rank Selectbox
	(getControl(2620,2634)) ctrlEnable false; //Update Rank button
};

if(life_my_gang_rank < 4) then {
...
lbClear _members;
{
	...
	_members lbAdd format["%1 - Rank: %2",(_x getVariable["realname",name _x]),(_x getVariable["gangrank",1])];
	_members lbSetData [(lbSize _members)-1,str(_x)];
	//};
} foreach (units grpPlayer);

_ranks = getControl(2620,2633);
lbClear _ranks;
for "_i" from 1 to 4 do {
	_ranks lbAdd format["%1",_i];
};
