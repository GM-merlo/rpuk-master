...
if(_action) then {
	if(RPUK_doshinbank < _upgradePrice) exitWith {
		hint parseText format[
			...
			[RPUK_doshinbank] call life_fnc_numberText,
			[_upgradePrice - RPUK_doshinbank] call life_fnc_numberText
		];
	};
	__SUB__(RPUK_doshinbank,_upgradePrice);
