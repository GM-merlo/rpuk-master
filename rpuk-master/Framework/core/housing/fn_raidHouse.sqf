...
*/
private["_uid","_cpRate","_cP","_title","_titleText","_ui","_houseInv","_houseInvData","_houseInvVal"];
params [
...
_value = 0;
{
	...
	if(_index != -1) then {
		_vIndex = [_var,dynamic_sell_array] call TON_fnc_index;
		if(_vIndex != -1) then {
			...
			_value = _value + (_val * round((dynamic_sell_array select _vIndex) select 1));
		};
	};
...
if(_value > 0) then {
	...
	hint format["The house had £%1 of illegal items inside.",[_value] call life_fnc_numberText];
	_house setVariable["Trunk",[_houseInvData,_houseInvVal],true];
	[[_house],"TON_fnc_updateHouseTrunk",(call life_fnc_findTransactionMethod),false] spawn life_fnc_MP;
} else {
