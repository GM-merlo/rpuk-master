...
//NDJ 16May14 Sort the lists alphabetically
lbSort _near;
lbSort _near_i;

_mstatus ctrlSetStructuredText parseText format["<img size='1.3' image='icons\bank.paa'/> <t size='0.8px'>£%1</t><br/><img size='1.2' image='icons\money.paa'/> <t size='0.8'>£%2</t>",[RPUK_doshinbank] call life_fnc_numberText,[RPUK_dosh] call life_fnc_numberText];
ctrlSetText[2009,format["Weight: %1 / %2", life_carryWeight, life_maxWeight]];
{
	...
	if(_val > 0) then
	{
		...
		if(_pic != "") then {
			_inv lbSetPicture [(lbSize _inv)-1,_pic];
		};
	};
