...
if((_ctrl == "pearl" || _ctrl == "clam") && {!(life_trunk_vehicle isKindOf "Ship")}) exitWith {hint "You cannot store that in anything but a ship!"};


if(_ctrl == "money") then
{
	_index = [_ctrl,_inv] call TON_fnc_index;
	if(RPUK_dosh < _num) exitWith {hint "You don't have that much cash on you to store in the vehicle!"};
	if(_index == -1) then
	{
