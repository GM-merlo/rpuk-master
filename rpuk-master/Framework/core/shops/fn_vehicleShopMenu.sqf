...
if(_shop == "") exitWith {};
if(
	!( _sideCheck isEqualTo	sideUnknown ) &&
	{ !( playerSide isEqualTo _sideCheck ) } &&
	{ !( str playerSide isEqualTo _sideCheck ) }
) exitWith {hint localize "STR_Shop_Veh_NotAllowed"};
...
if(_disableBuy || playerSide in [west,independent]) then {
...
//Loop through
{
  ...
  private _name = [(_vehicleInfo select 3), format["[%1] %2", _level, (_vehicleInfo select 3)]] select _locked;

	_control lbAdd _name;
	if(_vehicleInfo select 2 != "pictureThing") then {_control lbSetPicture [(lbSize _control)-1,(_vehicleInfo select 2)];};
