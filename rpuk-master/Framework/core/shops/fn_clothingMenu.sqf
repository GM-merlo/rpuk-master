...
if((_this select 3) in ["bhunter"] && !license_civ_bounty) exitWith {closeDialog 0;};
if((_this select 3) in ["wreb"] && !call life_fnc_isRebel) exitWith {hint localize "STR_Shop_NotaReb"; closeDialog 0;};
if((_this select 3) == "nhs" && playerSide != independent) exitWith {hint "You are not a NHS Paramedic"; closeDialog 0;};
*/

_title = getText(missionConfigFile >> "CfgClothingOldTonic" >> _this select 3 >> "title");
_openCond = getText(missionConfigFile >> "CfgClothingOldTonic" >> _this select 3 >> "openCondition");
if(!(call compile _openCond)) exitWith {hint "You do not have permission to use this clothing store."; closeDialog 0;};
ctrlSetText[3103,_title];
...
life_clothing_customCache = player getVariable["customUniform",["", -1]];
...
if(isNil "life_clothesPurchased") exitWith
{
	...
	if(life_oldClothes != "") then {
		...
		//NDJ 01May14 - make cops appear in skin-hack
	} else {
		removeUniform player;
	};

	if((life_clothing_customCache select 1) != -1 && (life_clothing_customCache select 0) == life_oldClothes) then {
		player setVariable["customUniform",life_clothing_customCache,true];
	} else {
		player setVariable["customUniform",nil, true];
	};
