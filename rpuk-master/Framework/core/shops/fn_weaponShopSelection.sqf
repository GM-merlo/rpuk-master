...
if((uiNamespace getVariable["Weapon_Shop_Filter",0]) == 1) then
{
	...
	//NDJ 1May14 - hack to make cop resale of items zero value
	if (playerSide == west || call life_fnc_isRebel) then {_price = 0;};
	...
	else
{
	...
	if(_price > RPUK_dosh) then
	{
		_priceTag ctrlSetStructuredText parseText format ["<t size='0.8'>Price: <t color='#ff0000'>£%1</t><br/>You lack: <t color='#8cff9b'>£%2</t></t>",[(_price)] call life_fnc_numberText,[(_price - RPUK_dosh)] call life_fnc_numberText];
	}
		else
	{
		_priceTag ctrlSetStructuredText parseText format ["<t size='0.8'>Price: <t color='#8cff9b'>£%1</t></t>",[(_price)] call life_fnc_numberText];
