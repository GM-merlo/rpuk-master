...
*/
private["_itemList","_index","_config","_priceTag", "_illegalItems"];
...
switch (_index) do
{
	case 0:
	{
		...
		{
      (["Weapons",_x select 0] call life_fnc_itemUnlocked) params [
        ["_locked", false, [false]],
        ["_level", 0, [0]]
      ];

      private _itemName = [_x select 1, _itemInfo select 1] select (isNil {_x select 1});
      private _text = [_itemName, format["[%1] %2", _level, _itemName]] select _locked;

      _itemInfo = [_x select 0] call life_fnc_fetchCfgDetails;
			_itemList lbAdd _text;
			_itemList lbSetData[(lbSize _itemList)-1,_itemInfo select 0];
			_itemList lbSetPicture[(lbSize _itemList)-1,_itemInfo select 2];
			_itemList lbSetValue[(lbSize _itemList)-1,ceil((_x select 2) * call life_priceModifier)];
	...
	case 1:
	{
		...
		_illegalItems = ["Binocular", "Rangefinder"];

		{
			if(!(_x in _listedItems) && _x != "" && (!(_x in _illegalItems))) then
			{
