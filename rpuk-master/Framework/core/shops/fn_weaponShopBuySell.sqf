...
(["Weapons",_item] call life_fnc_itemUnlocked) params [
  ["_locked", false, [false]],
  ["_level", 0, [0]]
];

if (_locked) exitWith {hint format["Sorry, you need to be level %1 to buy this.", _level]};
	...
	else
{
	if(!isNil {grpPlayer getVariable "gang_bank"} && {(grpPlayer getVariable "gang_bank") >= _price}) then {
		...
		if(_action) then {
			...
			if(_price > RPUK_dosh) exitWith {hint localize "STR_NOTF_NotEnoughMoney"};
			hint parseText format[localize "STR_Shop_Weapon_BoughtItem",_itemInfo select 1,[_price] call life_fnc_numberText];
			__SUB__(RPUK_dosh,_price);
			[_item,true] spawn life_fnc_handleItem;
			playSound "buy";
